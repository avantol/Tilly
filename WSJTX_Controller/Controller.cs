    ...
    public partial class Controller : Form
    {
        ...
        public Controller()
        {
            ...
            foreach (Control control in Controls) { 
                if (control.Location.Y >= callingLabel.Location.Y && control.Location.Y <= playSoundLabel.Location.Y)
                {
        ...
        protected override bool ProcessCmdKey(ref Message msg, Keys keyData)
        {
            ...
            {
                if (callListBox.SelectionMode != SelectionMode.None)
                {
                    wsjtxClient.NextCall(false, callListBox.SelectedIndex);
                    return true;
                }
        ...
        private void modeHelpLabel_Click(object sender, EventArgs e)
        {
            ...
            ShowHelp(
                ...
                $"{nl}Ctrl, P: Read number of pending 'Calls waiting reply'." +
        ...
        private void optionsButton_Click(object sender, EventArgs e)
        {
            ...
            ResumeLayout();
            if (showOptions) Show();
            if (!showOptions) statusText.Focus();
        }

        private void UpdateShowOptions()
        {
            optionsButton.Text = (showOptions ? "Hide options" : "Show all options");
            optionsButton.Visible = setupButton.Visible = showOptions || wsjtxClient.debug;
            verLabel.Visible = verLabel.Enabled = showOptions || wsjtxClient.debug;
            verLabel2.Visible = verLabel2.Enabled = showOptions || wsjtxClient.debug;
            ...
            if (showOptions)    //not sure why this is a problem with screen reader
            {
                exceptLabel.Text = "Block any reply to:";
            }
            else
            {
                exceptLabel.Text = "";
        ...
        {
            if (!formLoaded || e.KeyCode != Keys.Delete) return;
